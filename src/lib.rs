...
mod tests {
    ...
    fn test_chain(){
        ...
        let hideout_module = hideout.modules.iter().filter(|f| {
            f.module == test_hideout_module && f.level == level
        }).next().expect("Failed to find module");
            ...
            match requirements.require_type {
                Type::Item => {
                    match &requirements.name{
                        ...
                        Name::String(v) => {
                            let item = items.iter().filter(|i| &i.1.id == v).next().expect("Failed to find item by id");
                            println!("\t {{Item}} {} @ Level {}", item.1.name, requirements.quantity)
                        }
                    }
                }
                Type::Module => {
                ...
                Type::Trader => {
                    match &requirements.name{
                        Name::Integer(v) => {
                            let trader = traders.iter().filter(|t| {
                                t.1.id == *v
                            }).next().expect("Failed to get trader by id");

                            println!("\t {{Trader}} {}", trader.1.name);

                            let trader_req = &trader.1.loyalty[requirements.quantity as usize];
                            println!("\t\t\tLevel:\t{}", trader_req.level);
                            println!("\t\t\tRequired player level:\t{}", trader_req.required_level);
                            println!("\t\t\tRequired reputation:\t{}", trader_req.required_reputation);
                            println!("\t\t\tRequired sales [{}]:\t{}", trader.1.sales_currency, trader_req.required_sales);
